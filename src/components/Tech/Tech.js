import './Tech.css'
const Tech = () =>{
    return(
        <div className="tech">
            <h1>Technologies I'm Famlilar with</h1>
            <div className="box">
            <div className="icon">
            <img src="https://img.icons8.com/color/48/000000/java-coffee-cup-logo--v1.png" alt="java"/>
            <h3>JAVA</h3>
            </div>
            <div className="icon">
            <img src="https://img.icons8.com/color/48/000000/python--v2.png" alt="python"/>
            <h3>PYTHON</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/color/48/000000/c-plus-plus-logo.png" alt="C++"/>
            <h3>C++</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/color/48/000000/javascript--v1.png" alt="js"/>
            <h3>JS</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/ios/50/000000/php-logo.png" alt="php"/>
            <h3>PHP</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/nolan/48/react-native.png" alt="react-js"/>
            <h3>REACT JS</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/ios-filled/50/000000/github.png" alt="github"/>
            <h3>GITHUB</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/windows/48/000000/node-js.png" alt="nodejs"/>
            <h3>NODE</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/fluent/48/000000/docker.png"  alt="docker"/>
            <h3>DOCKER</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/ios/48/000000/mysql-logo.png" alt="mysql"/>
            <h3>MYSQL</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/color/48/000000/kubernetes.png" alt="KUBERNETES"/>
            <h3>KUBERNETES</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/ios-filled/48/000000/android-os.png" alt="android"/>
            <h3>ANDROID</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/color/48/000000/tensorflow.png" alt="tensorflow"/>
            <h3>TENSORFLOW</h3>
            </div>

            <div className="icon">
            <img src="https://img.icons8.com/ios-filled/48/000000/typescript.png" alt="typescript"/>
            <h3>TYPERSCRIPT</h3>
            </div>

            </div>
        </div>
    );
}

export default Tech;